...
const SOCIAL_PHOTOS: SocialPhoto[] = [
  { src: momentsTea, title: "Afternoon tea, just us", note: "Her favourite oolong, no rush at all. · from Instagram", daysAgo: 12 },
  { src: momentsHands, title: "Holding hands at the park", note: "She held on a little longer today. · from Instagram", daysAgo: 34 },
  { src: momentsGarden, title: "Back at the Botanic Gardens", note: "The orchids again — her happy place. · from Facebook", daysAgo: 61 },
  { src: clothesline05, title: "Her famous noodles", note: "She insisted on cooking for everyone again. · from Facebook", daysAgo: 90 },
];
...
export function ImportMomentsSheet({ open, onOpenChange, onImport }: Props) {
  ...
  const doImport = async () => {
    ...
    try {
      ...
      onImport(moments);
      onOpenChange(false);
      toast.success(`${moments.length} ${moments.length === 1 ? "memory" : "memories"} brought into the wall.`);
    } catch {
  ...
  return (
    <BottomSheet open={open} onOpenChange={(v) => stage === null && onOpenChange(v)} labelledBy="import-moments-title">
      <h2 id="import-moments-title" className="font-serif text-3xl italic font-light text-foreground/90">
        bring in memories you already have
      </h2>
      <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
        Years of photos already live on Instagram and Facebook. Pull the ones with{" "}
        the family into the memory wall.
      </p>
          ...
          {connected && stage === null && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              <p className="text-xs uppercase tracking-[0.14em] text-muted-foreground flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-sage inline-block" /> Connected · photos of the family
              </p>
              <div className="grid grid-cols-2 gap-2">
                {SOCIAL_PHOTOS.map((p, i) => {
                  const on = selected.has(i);
                  return (
                    <button key={p.title} onClick={() => toggle(i)} className="relative rounded-xl overflow-hidden border border-border text-left group">
                      <img src={p.src} alt={p.title} className="w-full aspect-square object-cover" />
                      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/60 to-transparent p-2 pt-6">
                        <p className="text-[11px] text-white/90 leading-tight line-clamp-2">{p.title}</p>
                      </div>
                      <span
