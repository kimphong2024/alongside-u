...
const rowToConsultation = (r: ConsultationRow): Consultation => ({
  ...
  status: (["processing", "ready", "failed"].includes(r.status)
    ? r.status
    : "failed") as ConsultationStatus,
...
export function useHealthData() {
  ...
  const updateConsultation = useCallback(
    async (
      id: string,
      patch: Partial<
        Pick<Consultation, "title" | "date" | "transcript" | "summary" | "actionSteps" | "status">
      >,
    ) => {
      setSnapshot({
      ...
      await supabase
        ...
        .update({
          ...
          ...(patch.actionSteps !== undefined
            ? { action_steps: patch.actionSteps as unknown as Json }
            : {}),
